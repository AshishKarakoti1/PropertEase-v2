    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (success) {
                ...
                // Store everything needed for the session
                localStorage.setItem('token', jwtToken);
                localStorage.setItem('loggedInUser', user.username);
                localStorage.setItem('user_email', user.email);
                localStorage.setItem('userId', user._id); // Useful for linking properties to users
    ...
    return (
        <div className="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 lg:px-8 mt-12 mb-0">
            <div className="relative max-w-lg mx-auto rounded-lg overflow-hidden">
                {/* Cleaned up the white background logic */}
                <div className="relative z-10 p-6 bg-white/90 backdrop-blur-sm shadow-2xl rounded-2xl border border-gray-100">
                    ...
                    <h1 className="text-center text-2xl font-bold text-indigo-600 sm:text-3xl mt-4">Welcome Back</h1>
                    ...
                    <form onSubmit={handleLogin} className="mb-0 mt-6 space-y-4">
                        ...
                        <div className="text-center space-y-2 pt-2">
                            <p className="text-sm text-gray-500">
                                No account? <Link to='/signup' className='text-indigo-600 font-semibold hover:underline'>Sign up</Link>
                            </p>
                            <p className="text-sm">
                                <Link to='/forgot-password' title="Reset Password" className='text-gray-400 hover:text-indigo-600 transition-colors'>Forgot Password?</Link>
                            </p>
                        </div>
                    </form>
                </div>
